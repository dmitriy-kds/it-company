...
class PrivateWorkerTests(TestCase):
    ...
    def test_worker_list_search_by_first_name(self):
        ...
        response = self.client.get(
            url,
            data={"first_or_last_name": self.another_worker.first_name}
        )
    ...
    def test_worker_list_search_by_last_name(self):
        ...
        response = self.client.get(
            url,
            data={"first_or_last_name": self.another_worker.last_name}
        )
    ...
    def test_worker_create_post_with_valid_data(self):
        ...
        self.assertRedirects(
            response,
            expected_url=reverse("task_manager:worker-list")
        )
    ...
    def test_worker_update_returns_200(self):
        url = reverse(
            "task_manager:worker-update",
            kwargs={"pk": self.another_worker.pk}
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_worker_delete_post(self):
        ...
        another_worker = get_user_model().objects.filter(
            username="another_username"
        )
