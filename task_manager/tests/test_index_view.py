...
class IndexTests(TestCase):
    ...
    def test_index_context(self):
        response = self.client.get(reverse("task_manager:index"))
        self.assertEqual(
            response.context["num_employees"],
        ...
        self.assertEqual(
            response.context["num_new_tasks"],
            Task.objects.filter(status="New").count())
        self.assertEqual(
            response.context["num_completed_tasks"],
            Task.objects.filter(status="Done").count())
        self.assertEqual(
            response.context["num_in_progress_tasks"],
            Task.objects.filter(status="In Progress").count())
